...
# Vendored from arwtyxouymz/modern-python-guidelines at commit 7501dd4, MIT licensed (the LICENSE
# file sits in the skill folder). Upstream source:
# https://raw.githubusercontent.com/arwtyxouymz/modern-python-guidelines/7501dd4/plugins/modern-python-guidelines/skills/use-modern-python/scripts/modern_python.py
# Vendored on 2026-09-17 with the following edits, kept small so that diffs against upstream stay
# readable:
#  1. `fix` and `check` require explicit paths; no path is a usage error, never a working-directory sweep.
#  2. The bundled-reference comparison in `list` and its helpers are deleted, so nothing downloads Ruff for a comparison; the exit-3 stop for a project Ruff too old to emit rule JSON and `--allow-stale` remain.
#  3. With no project configuration and no `--target-version`, the target source reads "Ruff default (no project configuration found)" and a warning asks for the target to be confirmed; the local interpreter is never consulted.
#  4. UTF-8 everywhere: subprocess output is decoded as UTF-8 with replacement, and stdout and stderr are reconfigured to UTF-8 at the top of `main`.
#  5. The em dash in the text renderer is replaced with a comma.
#  6. `check` and `fix` pass `--no-cache` to Ruff, so no `.ruff_cache/` is created in the project.
#  7. The Ruff pin is the constant PINNED_RUFF; the `ruff-fallback.txt` sidecar read is gone.
#  8. The `--unsafe-fixes` option is removed; `fix` applies Ruff's safe fixes only.
...
DEFAULT_TARGET_WARNING = (
    "The target Python version was not declared by the project, so Ruff's default was used. "
    "Confirm the target with the user or pass --target-version explicitly."
)
    ...
    if uvx := shutil.which("uvx"):
        candidates.append(
            RunnerCandidate(
                ...
                (uvx, "--from", f"ruff=={PINNED_RUFF}", "ruff"),
                "bundled",
            )
        )
    return candidates
...
def discover_runner(root: Path) -> Runner:
    ...
    attempted = "\n  - ".join(failures)
    raise ToolError(
        "Ruff is unavailable. Install Ruff in the project or install uv for the "
        f"bundled fallback. Attempts:\n  - {attempted}"
    )


def invoke_ruff(runner: Runner, arguments: Sequence[str]) -> subprocess.CompletedProcess[str]:
    result = run_process((*runner.argv, *arguments), cwd=runner.root)
...
def resolve_settings(
    ...
    *,
    ...
    if target_source == "Ruff resolved target":
        # Ruff prints "Settings path" only when it found a configuration file. Without one the
        # target is Ruff's own default, which the project never declared: say so, and let the
        # commands warn. The local interpreter is never consulted.
        target_source = config_path.name if config_path is not None else DEFAULT_TARGET_SOURCE
...
def settings_warnings(settings: ResolvedSettings) -> list[str]:
    if settings.target_source == DEFAULT_TARGET_SOURCE:
        return [DEFAULT_TARGET_WARNING]
    return []
...
def remaining_diagnostics(output: str) -> bool:
    try:
        diagnostics = json.loads(output)
    except json.JSONDecodeError as exc:
        raise ToolError(f"Ruff returned invalid diagnostic JSON: {exc}") from exc
    if not isinstance(diagnostics, list):
...
def command_probe(args: argparse.Namespace, runner: Runner) -> None:
    ...
    payload = {
        ...
        "warnings": [*runner.warnings, *settings_warnings(settings)],
    }
    print_json(payload)
...
def degraded_list_payload(
    ...
    return {
        ...
        "baseline": [],
        "conditional": [],
        "warnings": [*runner.warnings, *settings_warnings(settings), warning],
    }
...
def command_list(args: argparse.Namespace, runner: Runner) -> None:
    ...
    baseline_codes = guidelines.baseline_codes & policy.allowed
    conditional_codes = guidelines.conditional_codes & policy.allowed
...
def build_parser() -> argparse.ArgumentParser:
    ...
    check_help = (
        "Return modernization diagnostics as JSON; exit 1 when any remain, 0 when none, "
        "2 on a tool error"
    )
    check = subparsers.add_parser("check", help=check_help, description=check_help)
    add_rule_options(check)
    check.add_argument(
        "paths",
        nargs="+",
        help="Files or directories to check (required; the working directory is never swept)",
    )

    fix_help = (
        "Apply Ruff's safe fixes and return the remaining diagnostics as JSON; exit 1 when any "
        "remain, 0 when none, 2 on a tool error"
    )
    fix = subparsers.add_parser("fix", help=fix_help, description=fix_help)
    add_rule_options(fix)
    fix.add_argument(
        "paths",
        nargs="+",
        help="Files or directories to fix (required; the working directory is never swept)",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "list":
            ...
            status = command_check(args, runner, fix=False)
        elif args.command == "fix":
            status = command_check(args, runner, fix=True)
