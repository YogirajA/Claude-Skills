...
### Option A: Use the ECC hook (NOT available in this collection)

> **Adapted 2026-09-03.** Upstream ships an implementation at
> `scripts/hooks/gateguard-fact-force.js` inside the ECC plugin. **That file was not
> imported**, so this skill arrives as the *design* (gate stages, fact lists, tuning
> surface) and not as a runnable hook. Two ways to actually run it:
>
> 1. **Install the upstream package** (Option B below), which is self-contained.
> 2. **Port the three gates into `onboard-repo/library/hooks/`** as a Python
>    `PreToolUse` hook alongside `protect-paths.py` and `block-destructive.py`,
>    following the same fail-open discipline (exit 0 on unparseable input, and on
>    valid JSON of the wrong shape). The three-stage DENY / FORCE / ALLOW contract and
>    the per-gate fact lists above are the spec for that port.
>
> Everything below documents upstream's `ECC_*` environment surface. Those variables
> do nothing here until one of the two options above is done; they are kept because
> they are the best statement of which knobs a gate like this needs.
