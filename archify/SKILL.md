...
metadata:
  ...
  imported: "2026-09-07: copied from https://github.com/tt-a1i/archify at 920543b (v2.17, MIT), the archify/ skill folder only. Removed: the update checker (scripts/check-update.mjs, scripts/update-contract.mjs, skill-release.json, and the 'Update awareness' section of this file, which fetched a remote manifest after the first diagram), the upstream test suite, and the rendered example HTML (the JSON examples this file reads are kept). Vendored rather than installed with npx skills, because third-party plugin sources are out of policy here. Still needs Node on the path; brands capture fetches the URL you give it, and visual-check drives a real browser. Em dashes in the docs, the template comments and the example titles replaced on 2026-09-17."
---
