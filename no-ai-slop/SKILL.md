...
metadata:
  version: "1.0.6"
  author: petergyang
  origin: petergyang/no-ai-slop
  imported: "2026-09-12: copied from https://github.com/petergyang/no-ai-slop at 000650b (v1.0.6, MIT), three files only: skills/no-ai-slop/SKILL.md, skills/no-ai-slop/eval.md and LICENSE. Left out: the Codex plugin packaging (.codex-plugin/plugin.json, agents/openai.yaml, scripts/build_plugin.py, .github/workflows/plugin.yml), the logo (assets/no-ai-slop.png), PRIVACY.md, TERMS.md, plugin-submission.md, README.md and .gitignore. One modification: the em-dash rule (the 'Em dashes' paragraph in this file and check 9 of eval.md), which upstream relaxes to 1-2 in longer drafts, is tightened to none at all so it agrees with the global writing rule here. Everything else is verbatim; upstream contained no em dashes to strip. Vendored rather than installed with npx skills, because third-party plugin sources are out of policy here. No scripts, no Node, no network calls."
---
...
**Em dashes.** Do not use them at all, in short copy or long. Replace each one with a comma, a colon, parentheses, or a restructured sentence. En dashes for date and number ranges are fine.
